...
const QuestionsDetails = () => {
  const { id } = useParams();  // Get question ID from URL params
    ...
    const fetchQuestionDetails = async () => {
      try {
        const res = await axios.get(`http://localhost:3000/questions/${id}`, {
          headers: {
            'auhtentication': `${localStorage.getItem('token')}`  // Add JWT token in headers
          }
        });
        setQuestion(res.data);  // Set fetched question data
  ...
  const handleSubmitAnswer = async (e) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:3000/questions/${id}/answer`, 
        { answerText },
        {
          headers: {
      ...
      if (res.status === 201) {
        // Refetch question to get updated answers after successful submission
        setQuestion(res.data);  // Update the question data
        setAnswerText("");  // Clear answer input field
      }
    } catch (err) {
                    ...
                    <div>
                      ...
                      <Link to='/Users' className='user-link' style={{ color: '#000000' }}>
                        <Avatar backgroundColor='orange' px='8px' py='5px'>
                          {question.author ? question.author.charAt(0).toUpperCase() : 'U'}
                        </Avatar>
                        <div>
                          {question.author ? question.author : 'Unknown User'}
                        </div>
                      </Link>
                    </div>
                  </div>
